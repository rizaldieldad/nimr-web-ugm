...
// Initialize case6 page2 answers if not exists
if (!surveyState.answers["case6"]) {
    surveyState.answers.case6 = {
        q3: null
    }
}
...
const selectAnswer = (questionNumber, value) => {
    surveyState.answers.case6[`q${questionNumber}`] = value
}

// Function to check if a value is selected
const isSelected = (questionNumber, value) => {
    return surveyState.answers.case6[`q${questionNumber}`] === value
}

// Computed property to checck if all questions are answered
const isNextDisabled = computed(() => {
    return surveyState.answers.case6.q3 === null
})
</script>
    ...
    <div class="max-w-4xl mx-auto flex flex-col justify-center min-h-screen p-4 md:p-8 space-y-6">
        ...
        <div class="bg-white rounded-3xl shadow-lg p-6 md:p-8 transition-all hover:shadow-xl">
            ...
            <div class="flex justify-center items-center gap-3 md:gap-6">
                ...
                <button @click="selectAnswer(3, 'yes')" :class="[
                    'px-12 py-2 border font-bold rounded-full cursor-pointer transition-all',
                    isSelected(3, 'yes')
                        ? 'bg-pink-300 border-pink-300 text-white'
                        : 'border-pink-300 hover:bg-pink-300 text-gray-700'
        ...
        <div class="flex justify-end gap-6">
            <!-- Back Button -->
            <BackButton back-route="/survey/case6"/>

            <!-- Next Button -->
            <NextButton :disabled="isNextDisabled" class="justify-end" next-route="/survey/case7"/>
        </div>
    </div>
</template>
